#!/usr/bin/env python3
"""Block markup helpers for Industry's pattern generators.
...
def button(text, url="#", style=None, width=None, extra_class=None, background=None, text_color=None):
    ...
    needs neither — theme.json styles it `accent` with an `on-accent` label —
    but a button sitting on an `accent` ground inherits a fill identical to what
...
def separator(style="industry-bar", align=None):
    data = {"className": "is-style-" + style}
...
def cover(inner, slug, overlay="dark", dim=60, min_height=None, align="full",
    ...
    url = theme_image(slug)
    attr_json = attrs(data).replace('"INDUSTRY_IMAGE"', '"%s"' % url)
