 ...
 *
 *   WP_URL=http://127.0.0.1:9493 node .dev/editor-check.mjs
...
const only = process.env.PATTERNS ? process.env.PATTERNS.split( ',' ).map( ( s ) => 'industry/' + s ) : null;
...
const patterns = await page.evaluate( async () => {
	...
	return all.filter( ( p ) => 0 === p.name.indexOf( 'industry/' ) ).map( ( p ) => ( { name: p.name, content: p.content } ) );
} );
...
for ( const pattern of patterns ) {
	...
	const result = await page.evaluate( () => {
		...
		const icons = [ ...doc.querySelectorAll( '[class*="industry-icon--"], .industry-header__square' ) ];
	...
	checked++;
	const file = join( shots, pattern.name.replace( 'industry/', '' ) + '.png' );
