#!/usr/bin/env python3
"""
Fail on any `industry-*` CSS selector that nothing in the theme emits.
...
`'industry-field--' . esc_attr( $name )` or JavaScript such as
`'industry-' + name` registers its literal prefix, and any selector starting
with that prefix is treated as live. Without that, `.industry-field--message`
...
TOKEN = re.compile(r"industry-[a-z0-9_-]+")
SELECTOR = re.compile(r"\.(industry-[a-z0-9_-]+)")
# A runtime prefix is the industry- token that ENDS a string literal and is then
# concatenated with a variable. It must not be required to START the literal:
#
#   class="industry-field industry-field--' . esc_attr( $name ) . '"
#
# begins at `class="`, so the first version of this pattern — which wanted a
# quote directly before the prefix — missed `industry-field--` and reported the
# live `.industry-field--message` as dead. Its own first run caught that.
PHP_PREFIX = re.compile(r"""(industry-[a-z0-9_-]*)['"]\s*\.\s*(?:[a-z_]+\s*\(\s*)?\$""")
# 'industry-' + name   /   `industry-${name}`
JS_PREFIX = re.compile(r"""(industry-[a-z0-9_-]*)(?:['"]\s*\+|\$\{)""")
...
REGISTERED = re.compile(r"""array\(\s*'(industry-[a-z0-9-]+)'\s*,\s*__\(""")
...
def main():
    ...
    for path in EMITTERS:
        ...
        # block's comment (an icon class on an empty paragraph) reads as
        # `industry-icon\u002d\u002dtool` until it is decoded.
        text = open(path, encoding="utf-8").read().replace("\\u002d", "-")
        literal.update(TOKEN.findall(text))
        prefixes.update(p for p in PHP_PREFIX.findall(text) + JS_PREFIX.findall(text) if p)

    # A bare `industry-` prefix (an id built as 'industry-' . $form) would make
    # every selector count as live and the whole check pass vacuously.
    prefixes = {p for p in prefixes if p.rstrip("-") != "industry"}
    ...
    print("%d industry-* selectors, %d emitted literally, dynamic prefixes: %s"
