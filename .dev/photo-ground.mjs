 ...
 *
 *   INDUSTRY_PALETTE=colors-7-midnight node .dev/contrast-rendered.mjs
 */
export async function paletteCss( slug ) {
	...
	// In dark mode a palette only reaches the slugs scheme.css leaves alone.
	// Read them from the file: a hard-coded list is another theme's vocabulary
	// the moment this checker is ported, and it silently overrode the lifted
	// `accent` with the palette's own.
	const scheme = readFileSync( join( dirname( fileURLToPath( import.meta.url ) ), '..', 'assets', 'css', 'scheme.css' ), 'utf8' );
	const lifted = new Set( [ ...scheme.matchAll( /--wp--preset--color--([a-z0-9-]+):/g ) ].map( ( m ) => m[ 1 ] ) );
	const kept = palette.filter( ( p ) => ! lifted.has( p.slug ) )
		.map( ( p ) => `--wp--preset--color--${ p.slug }:${ p.color}` ).join( ';' );
	return `html:not(.industry-dark){${ all }} html.industry-dark{${ kept }}`;
}
