 ...
 *
 *   WP_URL=https://colorlibhub.com/industry node .dev/button-boundary.mjs
 *   INDUSTRY_DARK=1 WP_URL=… node .dev/button-boundary.mjs
 *   INDUSTRY_PATHS=/,/about/ WP_URL=… node .dev/button-boundary.mjs
 *
...
const dark = process.env.INDUSTRY_DARK === '1';
const paths = ( process.env.INDUSTRY_PATHS || '/,/about/,/services/,/projects/,/contact/,/blog/,/bank-holiday-press-rebuild/,/category/case-studies/,/?s=steel,/no-such-page/' ).split( ',' );
...
if ( process.env.INDUSTRY_PALETTE ) {
	...
	const buttons = await page.evaluate( () => {
		return [ ...document.querySelectorAll( '.wp-block-button__link, .wp-element-button' ) ]
			...
			.filter( ( el ) => el.offsetWidth > 0 && ! el.closest( '.industry-scheme-toggle' ) )
			.map( ( el, i ) => {
