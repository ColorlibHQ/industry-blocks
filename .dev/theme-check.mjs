 ...
 * Playground for this mounts the build output as `industry` and installs the
 * plugin from the directory:
 *
 *   bash .dev/build-zip.sh "$SCRATCH/industry-build"
 *   npx -y @wp-playground/cli@3.1.54 server --port=9493 --php=8.3 --wp=latest \
 *     --mount-before-install="$SCRATCH/industry-build/industry:/wordpress/wp-content/themes/industry" \
...
const theme = process.env.THEME || 'industry';
