 ...
 * the strip.
 *
 *   WP_URL=... INDUSTRY_PATHS=/,/about/ node .dev/contrast-rendered.mjs
 *   INDUSTRY_DARK=1 WP_URL=... node .dev/contrast-rendered.mjs
 *
 * @package Industry
 */
...
const paths = ( process.env.INDUSTRY_PATHS || process.env.INDUSTRY_URL || '/,/about/,/services/,/projects/,/contact/,/blog/,/bank-holiday-press-rebuild/,/category/case-studies/,/?s=steel,/no-such-page/' ).split( ',' ).filter( Boolean );
...
if ( process.env.INDUSTRY_PALETTE ) {
	await page.addStyleTag( { content: await paletteCss( process.env.INDUSTRY_PALETTE ) } );
	await page.waitForTimeout( 200 );
}

// INDUSTRY_DARK=1 measures the same page with dark mode on, which is where the
// palette is lifted rather than replaced and is the likeliest place for a
// pairing to fall under AA.
if ( process.env.INDUSTRY_DARK ) {
	await page.evaluate( () => {
		document.documentElement.classList.add( 'industry-dark' );
	} );
...
console.log( `\n${ paths.length } pages, ${ measuredTotal } text nodes, all at AA${ process.env.INDUSTRY_DARK ? ' (dark mode)' : '' }` );
