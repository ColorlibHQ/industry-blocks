...
( function () {
	...
	var KEY = 'industry-scheme';
	var root = document.documentElement;
	var strings = window.industryScheme || { toDark: 'Switch to dark mode', toLight: 'Switch to light mode' };

	function isDark() {
		return root.classList.contains( 'industry-dark' );
	}
	...
	function apply( dark ) {
		root.classList.toggle( 'industry-dark', dark );
	...
	function init() {
		...
		// started.
		var buttons = document.querySelectorAll( '.industry-scheme-toggle .wp-block-button__link' );
		...
		try {
			if ( ! localStorage.getItem( KEY ) && window.matchMedia ) {
				window.matchMedia( '(prefers-color-scheme: dark)' ).addEventListener( 'change', function ( e ) {
					...
					root.classList.toggle( 'industry-dark', e.matches );
					Array.prototype.forEach.call( buttons, label );
				} );
			}
		} catch ( e ) {}
