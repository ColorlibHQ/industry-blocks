 ...
 * call our filter.
 */
const INDUSTRY_UPDATE_HOST     = 'updates.colorlib.com';
const INDUSTRY_UPDATE_ENDPOINT = 'https://updates.colorlib.com/theme/industry.json';
const INDUSTRY_UPDATE_CACHE    = 'industry_update_response';
 ...
 * @return bool
 */
function industry_updates_enabled() {
	/**
	 * Filters whether Industry checks for updates.
	 *
	 ...
	 */
	return (bool) apply_filters( 'industry_check_for_updates', true );
}
 ...
 * @return array
 */
function industry_update_payload() {
	$payload = array(
		'theme'      => 'industry',
	 ...
	 */
	return apply_filters( 'industry_update_payload', $payload );
}
 ...
 * @return array|null
 */
function industry_fetch_update() {
	if ( ! industry_updates_enabled() ) {
		return null;
	}

	$cached = get_site_transient( INDUSTRY_UPDATE_CACHE );
	...
	if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
		set_site_transient( INDUSTRY_UPDATE_CACHE, array(), HOUR_IN_SECONDS );
		return null;
	}
	...
	set_site_transient( INDUSTRY_UPDATE_CACHE, $data, 12 * HOUR_IN_SECONDS );
 ...
 * @return array|false
 */
function industry_check_update( $update, $theme_data, $theme_stylesheet ) {
	// Recognised by its Update URI rather than its directory name: every
	// Colorlib theme shares this host, and a site may install Industry under
	// another directory (the colorlibhub demo uses themes/industry-blocks).
	if ( $update || empty( $theme_data['UpdateURI'] ) || INDUSTRY_UPDATE_ENDPOINT !== $theme_data['UpdateURI'] ) {
		return $update;
	}

	$release = industry_fetch_update();

	if ( ! $release || version_compare( $release['version'], INDUSTRY_VERSION, '<=' ) ) {
		return $update;
	}

	return array(
		'id'           => INDUSTRY_UPDATE_ENDPOINT,
		'theme'        => $theme_stylesheet,
		'version'      => $release['version'],
		'url'          => isset( $release['url'] ) ? $release['url'] : 'https://colorlib.com/wp/themes/industry/',
...
add_filter( 'update_themes_' . INDUSTRY_UPDATE_HOST, 'industry_check_update', 10, 3 );

/**
 * Forget the cached response when someone asks WordPress to check again.
 */
function industry_flush_update_cache() {
	delete_site_transient( INDUSTRY_UPDATE_CACHE );
}
add_action( 'upgrader_process_complete', 'industry_flush_update_cache' );
add_action( 'after_switch_theme', 'industry_flush_update_cache' );
 ...
 * their server.
 */
function industry_updates_notice() {
	$screen = get_current_screen();

	// Appearance → Themes, where the theme is chosen: Industry has no settings
	...
	$payload = industry_update_payload();

	// Industry ships no companion plugin, so this speaks only for the theme. If one
	...
	$note = __( 'Industry checks %1$s for updates twice a day and sends %2$s. No personal data and no site name is sent, and the identifier is a one-way hash. Add the %3$s filter to switch it off.', 'industry' );
	?>
	<p class="description industry-updates-note">
		<?php
		printf(
			esc_html( $note ),
			'<code>' . esc_html( INDUSTRY_UPDATE_HOST ) . '</code>',
			'<code>' . esc_html( implode( ', ', array_keys( $payload ) ) ) . '</code>',
			'<code>industry_check_for_updates</code>'
		);
		?>
	</p>
	<style>.industry-updates-note { max-width: 70ch; margin-top: 18px; }</style>
	<?php
}
