 ...
 * WooCommerce support.
 *
 * A firm that sells spare parts, safety kit or training places ends up with
 ...
 */
function industry_woocommerce_setup() {
...
add_action( 'after_setup_theme', 'industry_woocommerce_setup' );
 ...
 */
function industry_woocommerce_styles() {
	if ( ! class_exists( 'WooCommerce' ) ) {
		return;
	}
	 ...
	 */
	if ( ! apply_filters( 'industry_load_woocommerce_styles', $needed ) ) {
		return;
	}
...
add_action( 'wp_enqueue_scripts', 'industry_woocommerce_styles', 20 );
 ...
 * @return int
 */
function industry_woocommerce_columns() {
	return 3;
}
add_filter( 'loop_shop_columns', 'industry_woocommerce_columns' );
