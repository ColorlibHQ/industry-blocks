 ...
 *
 * Plenty of browsing happens late — a night-shift engineer checking a quote on
 * a phone, a site manager reading case studies after dark. This
 * adds a toggle that a visitor controls, separate from whichever palette the
 * site owner chose.
 *
 * It **lifts the active palette rather than replacing it**. An Industry site set to
 * Steel stays blue in dark mode; only the grounds and the text invert. That
 ...
 *
 *     add_filter( 'industry_enable_dark_mode', '__return_false' );
 *
 * @return bool
 */
function industry_dark_mode_enabled() {
	return (bool) apply_filters( 'industry_enable_dark_mode', true );
}
 ...
 */
function industry_scheme_boot_script() {
	if ( ! industry_dark_mode_enabled() ) {
		return;
	}
	?>
<script>
( function () {
	try {
		var stored = localStorage.getItem( 'industry-scheme' );
		var system = window.matchMedia( '(prefers-color-scheme: dark)' ).matches;
		var dark = stored ? 'dark' === stored : system;
		document.documentElement.classList.toggle( 'industry-dark', dark );
...
add_action( 'wp_head', 'industry_scheme_boot_script', 1 );

/**
 * The stylesheet and the toggle's behaviour.
 */
function industry_scheme_assets() {
	if ( ! industry_dark_mode_enabled() ) {
		return;
	}
	...
	wp_localize_script(
		...
		'industryScheme',
		array(
			'toDark'  => __( 'Switch to dark mode', 'industry' ),
			'toLight' => __( 'Switch to light mode', 'industry' ),
		)
	);
}
add_action( 'wp_enqueue_scripts', 'industry_scheme_assets' );

/**
 * Turn any button carrying `industry-scheme-toggle` into the switch.
 *
 ...
 * @return string
 */
function industry_scheme_toggle_markup( $content, $block ) {
	$classes = isset( $block['attrs']['className'] ) ? $block['attrs']['className'] : '';
	if ( false === strpos( $classes, 'industry-scheme-toggle' ) ) {
		return $content;
	}
	...
	$content = str_replace(
		'<a ',
		'<a role="button" aria-pressed="false" aria-label="' . esc_attr__( 'Switch to dark mode', 'industry' ) . '" ',
		$content
	);

	return $content;
}
add_filter( 'render_block_core/button', 'industry_scheme_toggle_markup', 10, 2 );
