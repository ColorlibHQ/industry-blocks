 ...
 * quote form inline would freeze its output into the page permanently.
 * `[industry_form]` survives the round trip because a shortcode is
...
const INDUSTRY_SETUP_FLAG = 'industry_front_page_created';
 ...
 */
function industry_starter_pages() {
	return array(
		'home'     => array(
			'title'    => __( 'Home', 'industry' ),
			'pattern'  => 'industry/page-home',
			'template' => 'page-no-title',
		),
		'about'    => array(
			'title'    => __( 'About', 'industry' ),
			'pattern'  => 'industry/page-about',
			'template' => '',
		),
		'services' => array(
			'title'    => __( 'Services', 'industry' ),
			'pattern'  => 'industry/page-services',
			'template' => '',
		),
		'projects' => array(
			'title'    => __( 'Projects', 'industry' ),
			'pattern'  => 'industry/page-projects',
			'template' => '',
		),
		'blog'     => array(
			'title'    => __( 'Blog', 'industry' ),
			'pattern'  => '',
			'template' => '',
		),
		'contact'  => array(
			'title'    => __( 'Contact', 'industry' ),
			'pattern'  => 'industry/page-contact',
			'template' => '',
		),
	);
}
 ...
 * overwrite the first.
 */
function industry_create_front_page() {
	if ( get_option( INDUSTRY_SETUP_FLAG ) ) {
		return;
	}

	// Only on a site that has not been built yet: someone activating Industry
		...
		array(
			...
			'exclude'        => array_filter(
				array(
					...
					industry_page_exists( 'sample-page' ),
				)
			),
		)
	);
	...
	if ( count( $existing ) > 1 ) {
		update_option( INDUSTRY_SETUP_FLAG, 'skipped: site already had pages', false );
		return;
	}
	...
	foreach ( industry_starter_pages() as $slug => $page ) {
		if ( industry_page_exists( $slug ) ) {
			continue;
		}

		$content = '';
		if ( $page['pattern'] ) {
			$content = industry_pattern_content( $page['pattern'] );
			if ( '' === $content ) {
				continue;
			}
		}
	...
	industry_create_primary_menu( $created );
	...
	if ( $created ) {
		update_option( INDUSTRY_SETUP_FLAG, gmdate( 'c' ), false );
	}
}
add_action( 'after_switch_theme', 'industry_create_front_page' );
 ...
 * @return int
 */
function industry_page_exists( $slug ) {
	$ids = get_posts(
		array(
			'name'           => $slug,
			'post_type'      => 'page',
 ...
 */
function industry_create_front_page_retry() {
	if ( get_option( INDUSTRY_SETUP_FLAG ) || ! current_user_can( 'edit_theme_options' ) ) {
		return;
	}

	industry_create_front_page();
}
add_action( 'admin_init', 'industry_create_front_page_retry' );
...
function industry_pattern_content( $name, $seen = array() ) {
	...
	return (string) preg_replace_callback(
		...
		static function ( $matches ) use ( $seen ) {
			...
			$nested = industry_pattern_content( $attributes['slug'], $seen );
 ...
 */
function industry_create_primary_menu( $pages ) {
	if ( ! $pages ) {
		return;
	}

	$order = array( 'home', 'about', 'services', 'projects', 'blog', 'contact' );
	$items = '';
	...
	wp_insert_post(
		array(
			'post_title'   => __( 'Primary', 'industry' ),
			'post_name'    => 'primary',
